...
A Spring Boot microservice project for travel planning and itinerary generation.

## Demo Video

You can watch the project demo here:

https://www.youtube.com/watch?v=wWQLqgV3eEI
...
3. Run the termination script when you want to stop the services
   ```bash
   ./terminate.sh
   ```

4. If you need to stop any non-running service or reset the environment, use:
   ```bash
   ./run_non_running.sh
   ```

> The services rely on Docker and the local startup scripts in this repository. Run them from the project root.

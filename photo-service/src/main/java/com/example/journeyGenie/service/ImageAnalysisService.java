...
public class ImageAnalysisService {
    ...
    public ResponseEntity<?> analyzeImageContent(ImageAnalysisDTO imageAnalysisDTO, HttpServletRequest request) {
        ...
        String email = existingUser != null ? existingUser.getEmail() : null;
    ...
    private List<PhotoData> extractUserPhotos(UserResponseDTO user) {
        List<PhotoData> photos = new ArrayList<>();

        if (user.getTours() != null) {
            for (TourResponseDTO tour : user.getTours()) {
                if (tour.getDays() != null) {
                    for (DayResponseDTO day : tour.getDays()) {
                        if (day.getPhotos() != null) {
                            for (PhotoResponseDTO photo : day.getPhotos()) {
